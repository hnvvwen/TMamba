...
def segm_init_weights(m):#初始化神经网络权重的函数
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, z, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if self.if_abs_pos_embed:                  # True 
            ...
            x = torch.cat((z, x), dim=1)           # torch.Size([B, 320, 384])  这段代码为什么又把z和x连接起来变成x了？
...
#                )
                ...
                # 通过当前层
                hidden_states_tmp, residual_tmp = layer(
                    hidden_states, residual, inference_params=inference_params
                )

                # 计算变化量
                diff = (hidden_states_tmp - pre_hidden).pow(2).mean(dim=-1).sqrt()  # Euclidean norm across feature dim
                change = diff.mean()

                # 如果变化大于阈值则激活
                threshold = torch.quantile(change, 0.1)

                # 如果变化大于阈值则激活
                if change > threshold:
                    hidden_states, residual = hidden_states_tmp, residual_tmp
...
#                residual = residual_f + residual_b.flip([1])
            for i in range(len(self.layers) // 2):
                if self.if_rope:
                    hidden_states = self.rope(hidden_states)
                    if residual is not None and self.if_rope_residual:
                        print("residual is not None")
                        residual = self.rope(residual)
                    print("residual is None")

                ### === 前向 ===
                pre_hidden_f = hidden_states.clone()
                hidden_states_f_tmp, residual_f_tmp = self.layers[i * 2](
                    hidden_states, residual, inference_params=inference_params
             )
                #delta_f = (hidden_states_f_tmp - pre_hidden_f).pow(2).mean(dim=-1).sqrt().mean()
                delta_f = (hidden_states_f_tmp - pre_hidden_f).pow(2).mean(dim=-1).sqrt()  # [B, T]

                ## === 后向 ===
                pre_hidden_b = hidden_states.flip([1]).clone()
                residual_b_input = None if residual is None else residual.flip([1])
                hidden_states_b_tmp, residual_b_tmp = self.layers[i * 2 + 1](
                    hidden_states.flip([1]), residual_b_input, inference_params=inference_params
            )
                #delta_b = (hidden_states_b_tmp - pre_hidden_b).pow(2).mean(dim=-1).sqrt().mean()
                delta_b = (hidden_states_b_tmp - pre_hidden_b).pow(2).mean(dim=-1).sqrt()
                delta_all = torch.cat([delta_f.flatten(), delta_b.flatten()], dim=0)  # [2 * B * T]
                ### === 判断是否激活 ===
                threshold = torch.quantile(delta_all, 0.5)
                #threshold = torch.quantile(torch.stack([delta_f, delta_b]), 0.2)
#                activate_f = delta_f > threshold
#                activate_b = delta_b > threshold
                activate_f = delta_f.mean() > threshold
                activate_b = delta_b.mean() > threshold

                # 如果变化显著则激活当前方向的层
                if activate_f:
                    hidden_states_f, residual_f = hidden_states_f_tmp, residual_f_tmp
                else:
                    hidden_states_f, residual_f = hidden_states, residual  # skip前向

                if activate_b:
                    hidden_states_b, residual_b = hidden_states_b_tmp, residual_b_tmp
                else:
                    residual_b = None if residual is None else residual.flip([1])
                    hidden_states_b = hidden_states.flip([1])


                # 合并双向结果（后向flip回来）
                hidden_states = hidden_states_f + hidden_states_b.flip([1])
                if residual_f is None and residual_b is None:
                    residual = None
                elif residual_f is None:
                    residual = residual_b.flip([1])
                elif residual_b is None:
                    residual = residual_f
                else:
                    residual = residual_f + residual_b.flip([1])
