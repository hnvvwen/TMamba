...
from lib.train.actors import TrackingMambaActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "trackingmamba":
    ...
    if settings.script_name == "trackingmamba":
        ...
        actor = TrackingMambaActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
